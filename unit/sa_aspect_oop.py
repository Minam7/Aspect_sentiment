...
class Data:
    def __init__(self, domain, data_dir, flag_word2vec, label_dict, seq_max_len, flag_addition_corpus,
        ...
        self.word_dict, self.word_dict_rev, self.embedding, aspect_list = ut.load_data(
            self.domain,
            self.data_dir,
            self.flag_word2vec,
            self.label_dict,
...
class Model:
    def __init__(self, batch_size, seq_max_len, nb_sentiment_label, nb_sentiment_for_word, embedding_size,
                 nb_linear_inside,
                 nb_lstm_inside, layers, TRAINING_ITERATIONS, LEARNING_RATE, WEIGHT_DECAY, flag_train,
                 flag_use_sentiment_for_word, sess):
    ...
    def modeling(self):
        self.tf_X_train = tf.placeholder(tf.float32, \
                                         shape=[None, self.seq_max_len, self.embedding_size - int(
                                             self.flag_use_sentiment_for_word) * self.nb_sentiment_for_word])
        ...
        self.ln_w = tf.Variable(tf.truncated_normal([self.embedding_size, self.nb_linear_inside], stddev=math.sqrt(
            3.0 / (self.embedding_size + self.nb_linear_inside))))
        ...
        self.sent_w = tf.Variable(tf.truncated_normal([self.nb_lstm_inside, self.nb_sentiment_label],
                                                      stddev=math.sqrt(
                                                          3.0 / self.nb_lstm_inside + self.nb_sentiment_label)))
        ...
        self.cross_entropy = tf.reduce_mean(
            tf.multiply(tf.nn.softmax_cross_entropy_with_logits(logits=sentiment, labels=y_labels),
                        self.tf_X_train_mask))
        ...
        self.correct_prediction = tf.reduce_sum(
            tf.multiply(tf.cast(tf.equal(self.prediction, self.tf_y_train), tf.float32), self.tf_X_binary_mask))
        self.global_step = tf.Variable(0, trainable=True)
        self.learning_rate = tf.train.exponential_decay(self.LEARNING_RATE, self.global_step, 1000, 0.65,
                                                        staircase=True)
    ...
    def evaluate(self, data, flag_write_to_file, flag_train):
        ...
        correct_prediction_test, prediction_test = self.sess.run([self.correct_prediction, self.prediction],
                                                                 feed_dict={self.tf_X_train: np.asarray(data.x_test),
                                                                            self.tf_X_binary_mask: np.asarray(
                                                                                data.test_binary_mask),
                                                                            self.tf_X_seq_len: np.asarray(
                                                                                data.test_seq_len),
                                                                            self.tf_X_sent_for_word: np.asarray(
                                                                                data.test_sentiment_for_word),
                                                                            self.tf_y_train: np.asarray(
                                                                                data.test_label),
                                                                            self.keep_prob: 1.0})
            ...
            for i in range(len(data.test_data)):
                ...
                for j in range(len(data.test_data[i])):
                    if data.word_dict_rev[data.test_data[i][j]] == '<unk>':
                        ...
                        data_sample = data_sample + data.word_dict_rev[data.test_data[i][j]] + '(label ' + str(
                            data.test_label[i][j]) + \
                                      '|predict ' + str(prediction_test[i][j]) + ') '
                    else:
                        data_sample = data_sample + data.word_dict_rev[data.test_data[i][j]] + ' '
        ...
        for it in range(self.TRAINING_ITERATIONS):
            ...
            self.sess.run(self.optimizer,
                          feed_dict={self.tf_X_train: np.asarray(data.x_train[index: index + self.batch_size]),
                                     ...
                                     self.tf_X_binary_mask: np.asarray(
                                         data.train_binary_mask[index: index + self.batch_size]),
                                     self.tf_X_seq_len: np.asarray(data.train_seq_len[index: index + self.batch_size]),
                                     self.tf_X_sent_for_word: np.asarray(
                                         data.train_sentiment_for_word[index: index + self.batch_size]),
