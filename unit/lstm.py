...
def tokenize_text(text, in_normalizer):
    text = text.replace('.', ' ')
    text = re.sub('\s+', ' ', text).strip()
    text = text.replace('\u200c', ' ').replace('\n', '').replace('\r', '').replace('ي', 'ی').replace('ك', 'ک')
    normalized_text = in_normalizer.normalize(text)
    tokens = word_tokenize(normalized_text)
    return tokens


def process_data(products, in_normalizer):
    categories_set = set()
    all_comments = []
    for product in products:
        ...
        for comment_dict in comments:
            ...
            if text is None:
                text = ''
            tokens = tokenize_text(text, in_normalizer=in_normalizer)
            all_comments.append({
                'pol': pol,
                'tokens': tokens
            })
...
def lstm():
    ...
    if is_training_data_ready:
        ...
        all_comments = process_data(products, normalizer)
    ...
    while interactive_mode:
        text = input('comment: ')
        tokens = tokenize_text(text, normalizer)
